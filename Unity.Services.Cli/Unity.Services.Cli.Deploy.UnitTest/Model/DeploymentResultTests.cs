...
namespace Unity.Services.Cli.Deploy.UnitTest.Model;
...
public class DeploymentResultTests
{
    ...
    {
        m_DeploymentResult = new DeploymentResult(k_DeployedContents, k_FailedContents);
        var result = m_DeploymentResult.ToString();

        Assert.IsTrue(result.Contains($"Successfully deployed the following contents:{System.Environment.NewLine}    {k_DeployedContents.First().Name}"));
    ...
    {
        m_DeploymentResult = new DeploymentResult(new List<DeployContent>(), new List<DeployContent>());
        var result = m_DeploymentResult.ToString();

        Assert.IsFalse(result.Contains($"Successfully deployed the following contents:{System.Environment.NewLine}    {k_DeployedContents.First().Name}"));
